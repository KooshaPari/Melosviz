...
export function savePlaybackVolume(state: PlaybackVolumeState): PlaybackVolumeState {
  const next = {
    volume: clampVolume(state.volume),
    muted: Boolean(state.muted),
  }
  if (typeof window !== 'undefined') {
    window.localStorage.setItem(PLAYBACK_VOLUME_STORAGE_KEY, JSON.stringify(next))
  }
  return next
}
