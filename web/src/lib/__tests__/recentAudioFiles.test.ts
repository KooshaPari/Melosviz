...
describe('recentAudioFiles', () => {
  ...
  it('persists path entries with name and lastUsed', () => {
    ...
    expect(JSON.parse(localStorage.getItem(RECENT_AUDIO_STORAGE_KEY) ?? '[]')).toHaveLength(1)
  })
  ...
  it('does not persist blob URLs for file entries', () => {
    ...
    const raw = JSON.parse(localStorage.getItem(RECENT_AUDIO_STORAGE_KEY) ?? '[]') as Array<{
      path?: string
    }>
  ...
  it('bumps existing entry to top on re-use', () => {
    ...
    const list = pushRecentAudioFile({ name: 'a.mp3', size: 100, kind: 'file' })
