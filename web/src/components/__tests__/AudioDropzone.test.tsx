...
describe('AudioDropzone', () => {
  ...
  it('renders drop zone and path input', () => {
    ...
    expect(screen.getByRole('group', { name: /drop an audio file/i })).toBeInTheDocument()
  })
  ...
  it('records path on blur', () => {
    ...
    const stored = JSON.parse(localStorage.getItem(RECENT_AUDIO_STORAGE_KEY) ?? '[]') as Array<{
      name: string
      kind: string
    }>
  ...
  it('labels file-only recent entries for re-pick', () => {
    ...
    expect(screen.getByRole('button', { name: /session\.mp3 \(re-pick file\)/i })).toBeInTheDocument()
  })
  ...
  it('clears recent list from storage', () => {
    localStorage.setItem(
      RECENT_AUDIO_STORAGE_KEY,
      JSON.stringify([
        { name: 'demo.wav', size: 1024, lastUsed: Date.now(), kind: 'path', path: '/tmp/demo.wav' },
      ]),
    )
    render(<AudioDropzone value="" onChange={vi.fn()} />)
    fireEvent.click(screen.getByRole('button', { name: /clear recent audio files list/i }))
    expect(localStorage.getItem(RECENT_AUDIO_STORAGE_KEY)).toBeNull()
    expect(screen.queryByRole('button', { name: 'demo.wav' })).not.toBeInTheDocument()
  })
})
