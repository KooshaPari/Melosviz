...
describe('PlaybackTransport', () => {
  it('exposes i18n aria-labels and visible time readout', () => {
    ...
    expect(screen.getByRole('button', { name: /reset playback to start/i })).toBeInTheDocument()
  ...
  it('calls transport handlers', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /reset playback to start/i }))
  ...
  it('localizes controls in Spanish', () => {
    setLocale('es')
    render(<PlaybackTransport {...baseProps} />)
    expect(screen.getByRole('button', { name: /iniciar reproducción/i })).toBeInTheDocument()
  ...
  it('renders volume/mute controls when showVolume is set', () => {
    ...
    expect(screen.getByRole('button', { name: /mute track audio/i })).toBeInTheDocument()
  ...
  it('renders rate preset buttons and calls onPlaybackRateChange', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /set playback speed to 0\.5/i }))
    expect(onPlaybackRateChange).toHaveBeenCalledWith(0.5)

    fireEvent.click(screen.getByRole('button', { name: /set playback speed to 1\.5/i }))
