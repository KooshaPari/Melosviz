...
describe('LoadingOverlay', () => {
  it('does not render dialog when visible=false', () => {
    const { container } = render(<LoadingOverlay visible={false} onCancel={vi.fn()} />)
  ...
  it('calls onCancel on Escape', async () => {
    ...
    render(<LoadingOverlay visible={true} onCancel={onCancel} />)
    await waitFor(() => {
      expect(document.getElementById('loading-overlay-cancel')).toBe(document.activeElement)
    })
  ...
  it('uses static freq bars when prefers-reduced-motion is set', () => {
    ...
    const { container } = render(<LoadingOverlay visible={true} onCancel={vi.fn()} />)
  ...
  it('announces percentage in live region when progressPct is provided', () => {
    render(<LoadingOverlay visible={true} onCancel={vi.fn()} progressPct={42} />)
    const live = document.querySelector('[aria-live="polite"]')
