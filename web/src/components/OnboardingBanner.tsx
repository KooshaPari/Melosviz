    ...
    >
      <div
        className={`pointer-events-auto max-w-sm rounded-xl border border-white/10 bg-black/55 px-5 py-4 shadow-xl ${
          reducedMotion ? '' : 'backdrop-blur-sm transition-opacity duration-500 ease-out'
        }`}
      ...
      >
        ...
        />
        <ol className="mt-3 flex justify-center gap-2" aria-label={t('onboarding.steps_label')}>
          {STEPS.map((key, i) => (
            <li
              key={key}
