...
export function AudioDropzone({ value, onChange, disabled = false }: AudioDropzoneProps) {
  const fileInputRef = useRef<HTMLInputElement>(null)
  const pendingRecentRef = useRef<RecentAudioEntry | null>(null)
  const [dragOver, setDragOver] = useState(false)
  const [recent, setRecent] = useState<RecentAudioEntry[]>(() => loadRecentAudioFiles())
  const reducedMotion = usePrefersReducedMotion()

  const remember = useCallback(
    (entry: Omit<RecentAudioEntry, 'lastUsed'>) => {
  ...
  const loadPath = useCallback(
    (path: string) => {
      ...
      if (!trimmed) return
      onChange(trimmed)
      remember({ name: basename(trimmed), size: 0, kind: 'path', path: trimmed })
    },
    [onChange, remember],
  )
    ...
    <div className="flex flex-col gap-2">
      ...
      {recent.length > 0 && (
        <div className="flex flex-col gap-1">
          ...
          <ul className="flex flex-col gap-0.5 max-h-28 overflow-y-auto" aria-label={t('audio.recent_title')}>
            {recent.map((entry) => {
              ...
              const label = needsRepick
                ...
                : entry.name
              return (
                <li key={`${entry.kind}-${entry.name}-${entry.size}-${entry.lastUsed}`}>
                  <button
                    type="button"
                    disabled={disabled}
                    onClick={() => handleRecentClick(entry)}
                    title={needsRepick ? t('audio.recent_repick_hint') : entry.path}
