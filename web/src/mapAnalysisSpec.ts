...
import { getSceneTemplate, PRESET_SCENE_FAMILIES, type SceneTemplateId } from './sceneTemplates'
...
export interface RawAnalysisSpec {
  ...
  palette?: string[]
  metadata?: { duration?: number; preset?: string; transition_fraction?: number }
...
function beatTimesFromRaw(raw: RawAnalysisSpec): number[] {
  ...
  const events = raw.timeline_events as Array<{ t?: number; type?: string }> | undefined
  if (events) {
...
export function mapAnalysisToRenderSpec(raw: RawAnalysisSpec): RenderSpec {
  ...
  const segments = (raw.scene_segments ?? raw.sceneSegments ?? []) as SceneSegment[]
  ...
  return {
    ...
    transitionFraction: (raw.metadata?.transition_fraction as number | undefined) ?? 0.12,
  }
}

/** Remap segment scene templates when a UI preset is applied. */
export function applyPresetToSpec(spec: RenderSpec, presetId: string): RenderSpec {
  ...
  const keyframes = remappedSegments.map((seg, i) => {
    ...
    return {
      ...
      color: existing?.color ?? {
        ...
        secondary: spec.palette?.[(i + 1) % (spec.palette?.length ?? 1)] ?? '#22d3ee',
        brightness: 0.7,
      },
