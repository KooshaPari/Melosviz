...
function keysWithPrefix(catalog: Record<string, string>, prefix: string): string[] {
  return Object.keys(catalog).filter((k) => k.startsWith(prefix)).sort()
}
...
describe('web i18n catalogs', () => {
  ...
  it('studio polish prefixes have matching en/es keys', () => {
    for (const prefix of STUDIO_POLISH_PREFIXES) {
      ...
      expect(enKeys.length, `no keys under prefix ${prefix}`).toBeGreaterThan(0)
    }
  })
  ...
  it('interpolates analysis server errors', () => {
    setLocale('en')
    expect(tf('error.analysis_server', { status: 500, statusText: 'Internal Server Error' })).toBe(
  ...
  it('interpolates preset duration label', () => {
    setLocale('en')
    expect(tf('preset.duration_bpm', { seconds: 180, bpm: 128 })).toBe('180s · 128 BPM')
  })
  ...
  it('interpolates spec summary and playback time readout', () => {
    ...
    expect(tf('playback.time', { elapsed: '1:00', total: '3:00' })).toBe('1:00 / 3:00')
  ...
  it('interpolates memory-cap hard error', () => {
    setLocale('en')
    expect(tf('error.memory_cap_hard', { rssMb: 200, capMb: 100 })).toContain('200')
    expect(tf('error.memory_cap_hard', { rssMb: 200, capMb: 100 })).toContain('100')
  })
  ...
  it('translates locale switcher strings in Spanish', () => {
    ...
    expect(tf('locale.switch_aria', { lang: t('locale.es') })).toContain('Español')
  })
