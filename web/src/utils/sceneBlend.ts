 ...
 */
export function resolveSceneBlend(
  spec: RenderSpec,
  playbackT: number,
): SceneBlendState {
  ...
  const fromTemplate = (current.scene_template ??
    "torus_flow") as SceneTemplateId;
  ...
  const transitionNorm = Math.min(
    0.35,
    (current.transition_secs ??
      spec.durationSecs * (spec.transitionFraction ?? 0.12)) /
  ...
  const sceneLabel =
    blend > 0.5
      ? (next.scene ?? sceneLabelFromTemplate(toTemplate))
      : (current.scene ?? sceneLabelFromTemplate(fromTemplate));
