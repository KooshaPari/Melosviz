...
class TestResolveFFmpegBinary:
    ...
    def test_skips_nonexistent_candidate(self, tmp_path):
        ...
        with (
            patch("shutil.which", return_value=fake),
            pytest.raises(FFMpegNotFoundError),
        ):
            _resolve_ffmpeg_binary()
    ...
    def test_skips_broken_binary(self, tmp_path):
        ...
        fake.chmod(0o755)
        with (
            patch("shutil.which", return_value=str(fake)),
            patch(
                "subprocess.run",
    ...
    def test_env_var_candidate_working(self, tmp_path):
        ...
        with (
            patch.dict("os.environ", {"MELOSVIZ_FFMPEG_BIN": str(fake)}),
...
class TestRawVideoPipeErrors:
    ...
    def test_communicate_exception_raises_render_export_error(self, tmp_path):
        ...
        with (
            ...
            pytest.raises(RenderExportError, match="streaming rawvideo"),
        ):
            _export_video_rawvideo_pipe(
                "ffmpeg",
                [(128, 0, 0)],
    ...
    def test_nonzero_returncode_raises_render_export_error(self, tmp_path):
        ...
        with (
            patch("subprocess.Popen", return_value=mock_proc),
            pytest.raises(RenderExportError, match="rawvideo export failed"),
        ):
            _export_video_rawvideo_pipe(
                "ffmpeg",
                [(128, 0, 0)],
    ...
    def test_empty_output_raises_render_export_error(self, tmp_path):
        ...
        with (
            patch("subprocess.Popen", return_value=mock_proc),
            pytest.raises(RenderExportError, match="no output was produced"),
        ):
            _export_video_rawvideo_pipe(
...
class TestExportVideoErrors:
    ...
    def test_unsupported_format_raises(self, tmp_path):
        ...
        with (
            ...
            pytest.raises(RenderExportError, match="Unsupported export format"),
        ):
    ...
    def test_png_path_nonzero_rc_raises_render_export_error(self, tmp_path):
        ...
        with (
            ...
            patch("subprocess.run", return_value=mock_result),
            pytest.raises(RenderExportError, match="ffmpeg export failed"),
        ):
            export_video(spec, format="mp4", output_dir=tmp_path)
    ...
    def test_duration_zero_uses_default(self, tmp_path):
        ...
        with (
            ...
            patch("subprocess.run", return_value=mock_result),
            pytest.raises(Exception),  # noqa: B017 -- exact failure type from ffmpeg subprocess mock is unspecified
        ):
            export_video(spec, format="mp4", output_dir=tmp_path)
...
class TestTDBridge:
    ...
    def test_stream_render_spec_batch_mode(self):
        ...
        bridge = TDBridge(BridgeConfig(transport="osc"))
        spec = self._make_spec()
        with patch.object(bridge, "_send_sync") as mock_send, patch("socket.socket"):
...
class TestTDAdapter:
    ...
    def test_live_mode_starts_bridge(self, tmp_path):
        ...
        with (
            patch.dict(
                ...
                {
                    "melosviz.runtime.touchdesigner.generator": MagicMock(
                        generate_network=MagicMock(return_value=mock_result)
                    )
                },
            ),
    ...
    def test_start_bridge_with_explicit_config(self, tmp_path):
        ...
        spec = self._make_spec()
        with patch("socket.socket"), patch("threading.Thread") as mock_thread_cls:
            mock_thread = MagicMock()
            mock_thread_cls.return_value = mock_thread
    ...
    def test_nested_path_overwrites_non_dict(self):
        ...
        net = {
            "groups": [
                {
                    "name": "g",
                    "operators": [{"name": "op", "params": {"nested": "old"}}],
                }
            ]
        }
    ...
    def test_diff_diverging(self):
        ...
        network = {
            "groups": [
                {
                    "name": "scanner",
                    "operators": [{"name": "main", "params": {"angle": 15}}],
                }
            ]
        }
    ...
    def test_diff_matching_value_excluded(self):
        ...
        network = {
            "groups": [
                {
                    "name": "scanner",
                    "operators": [{"name": "main", "params": {"angle": 21}}],
                }
            ]
        }
        overrides = {"scanner.main.angle": 21}
...
class TestOrchestrator:
    ...
    def test_assembly_encode_adapter_missing_raises(self, tmp_path):
        ...
        with (
            patch(
                "melosviz.conductor.registry.ADAPTER_REGISTRY",
                {"video_export": MagicMock()},
            ),
            pytest.raises((ConductorError, Exception)),
        ):
    ...
    def test_assembly_step_failure_raises(self, tmp_path):
        ...
        with (
            patch.object(reg_mod, "ADAPTER_REGISTRY", registry),
            pytest.raises(ConductorError, match="final assembly_encode step failed"),
        ):
...
class TestBridgeServer:
    ...
    def test_render_missing_file(self, client, tmp_path):
        resp = client.post(
            "/render", json={"wav_path": "/nonexistent.wav", "out_dir": str(tmp_path)}
        )
        assert resp.status_code == 400
        ...
        plan = {
            ...
            "segments": [
                ...
                },
                {
                    ...
                    "scene_type": "video_export",
                    "material": "neon_glow",
                    "camera_language": "cut_frenzy",
                    "intensity": 0.8,
                },
                {
                    "beat_aligned_start": 2.0,
                    "scene_type": "blender_3d",
                    "material": "pastel",
