...
def main() -> int:
    p = Path(
        os.environ.get(
            "MELOSVIZ_AUDIT_PATH",
            str(Path.home() / "CodeProjects" / "Phenotype" / "repos" / "MelosViz-audit.json"),
        )
    )
    if not p.exists():
        print(f"audit file not found: {p}", file=sys.stderr)
        return 1
    ...
    if isinstance(s.get("weaknesses"), list):
        ...
        for w in s["weaknesses"]:
            if isinstance(w, dict) and w.get("id") in [
                "W001",
                "W002",
                "W003",
                "W005",
    ...
    s["pillars_passing"] = sum(
        1
        for c in d["scores"].values()
        for pil in c.get("pillars", {}).values()
        if pil.get("v", 0) >= 1
    )
